    ...
    init {
        viewModelScope.launch {
            ...
            when (val result = companyInfoResult.await()) {
                is Resource.Error -> {
                    state = state.copy(
                        company = null, isLoading = false, error = result.message,
                    )
                }

                is Resource.Success -> {
                    state = state.copy(
                        company = result.data, isLoading = false, error = null
                    )
                }
            ...
            when (val result = intradayInfoResult.await()) {
                is Resource.Error -> {
                    state = state.copy(
                        stockInfo = emptyList(), isLoading = false, error = result.message,
                    )
                }
